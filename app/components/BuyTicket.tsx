...
export function BuyTicket({ onSuccess }: BuyTicketProps) {
  ...
  const handleMint = async () => {
    ...
    if (!isDemoMode) {
      ...
      if (balance && parseFloat(balance.formatted) < 0.1) {
        setError('Insufficient balance. You need at least 0.1 ETH to mint a ticket.');
        return;
      }
            ...
            <div className="mt-5 pt-4 border-t border-blue-100">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-2 rounded-lg shadow-sm">
                  ...
                  <div className="flex items-center">
                    <span className="text-xl sm:text-2xl font-bold text-gray-900">0.1 ETH</span>
                    {!isDemoMode && (
                ...
                {isConnected && balance && !isDemoMode && (
                  ...
                  >
                    ...
                    <span className={`text-sm font-medium ${parseFloat(balance.formatted) < 0.1 ? 'text-red-600' : 'text-gray-700'}`}>
